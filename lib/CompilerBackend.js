...
class SyntaxTree {
    ...
    static If = class {
        ...
        toXML() {
            return (compressed? "<i><c>":"<if><condition>") + putValue(this.cond) + (compressed? "</c><p>":"</condition><program>") + toXML(this.program, compressed, true) + (compressed? "</p>":"</program>") + (compressed? "</i>":"</if>");
        }
    }
}
