import { xmlToSyntaxTree } from "./XMLToSyntaxTree";
...
function putValue(value) {
    if (typeof value == 'number') {
        ...
        return "<text data=\"".replace("text", compressed? "t":"text") + value.replace('\n', '\\n') + "\"/>"
    } else {
        return value.toXML()
    }
}
...
class SyntaxTree {
    ...
    static Not = class {
        ...
        toXML() {
            return (compressed? "<not>":"<n1>") + putValue(this.value) + (compressed? "</not>":"</n1>")
        }
    }
    static BitwiseNot = class {
        value = null
        constructor(value) {
            this.value = value;
        }
        ...
        toXML() {
            return (compressed? "<bitwise-not>":"<bn>") + putValue(this.value) + (compressed? "</bitwise-not>":"</bn>")
        }
    }
    ...
    static Function = class {
        functionName = null
        program = null
        constructor(functionName, program, ...args) {
        ...
        toXML() {
            var args = (compressed? 'a=\"':'args=\"')
            for (var i of this.args) {
                args += i + ','
            }
            args = args.replace(/,$/g, '') + '"'
            return (compressed? "<fun n=\"":"<function name=\"") + this.functionName + '" ' + args + '>' + toXML(this.program, compressed, true) + (compressed? "</fun>":"</func>")
        }
    }
    static CallFunction = class {
        functionName = null
        ...
        toXML() {
            var args = '', c = -1
            for (var i of this.args) {
                args += '<arg' + ++c + '>' + putValue(i) + '</arg' + c + '>'
            }
            return (compressed? "<c n=\"":"<call-function name=\"") + this.functionName + "\">" + args + (compressed? "</c>":"</call-function>");
        }
    }
    static Increase = class {
        constructor(variable, isPostfix) {
            this.variable = variable
            this.isPostfix = isPostfix
        }

        toXML() {
            return (compressed? "<in p=\"":"<increase is-postfix=\"") + this.isPostfix + "\">" + putValue(this.variable) + (compressed? "</in>":"</increase>");
        }
    }
    static Decrease = class {
        constructor(variable, isPostfix) {
            this.variable = variable
            this.isPostfix = isPostfix
        }
        ...
        toXML() {
            return (compressed? "<de p=\"":"<decrease is-postfix=\"") + this.isPostfix + "\">" + putValue(this.variable) + (compressed? "</de>":"</decrease>");
        }
    }
}
